...
## Three plugins worth knowing about

These aren't recommendations for your setup — they're concrete examples of what a plugin looks like in practice.

**superpowers**
Anthropic's flagship plugin. Adds skills for planning, debugging, TDD, brainstorming, and spec-driven development. Each skill is a structured prompt sequence with clear inputs and outputs. Install it to see how a well-designed plugin feels.

**claude-mem**
Adds session memory — stores context from previous sessions so Claude can pick up where it left off. Useful if you find yourself re-explaining your project setup every session. Exposes a `mem-search` skill and a `/resume` command.

**lean-ctx**
Context engineering layer. Replaces standard file reads with cached, compressed equivalents. Saves tokens on large codebases. Lower-level than the others — good to understand if you care about context budget.
